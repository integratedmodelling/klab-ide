...
import org.integratedmodelling.klab.ide.cli.Command;
import org.integratedmodelling.klab.ide.cli.Option;
import org.integratedmodelling.klab.ide.cli.REPLTextField;
...
public class REPLTextFieldTest extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Label infoLabel = new Label("REPLTextField Test. Try typing 'help', 'list', 'run', or use Up/Down for history.");
    ...
    private List<Command> createMockCommands() {
        ...
        Command help = new Command("help", "Show help information");
        help.addOption(new Option("--verbose", "Show detailed help", false));
        commands.add(help);

        Command list = new Command("list", "List available resources");
        list.addOption(new Option("--type", "Filter by type", true));
        list.addOption(new Option("--all", "Show all items", false));
        commands.add(list);

        Command run = new Command("run", "Execute a process");
        run.addOption(new Option("--id", "Process ID", true));
        run.addOption(new Option("--now", "Run immediately", false));
        commands.add(run);

        Command exit = new Command("exit", "Close the application");
        commands.add(exit);
